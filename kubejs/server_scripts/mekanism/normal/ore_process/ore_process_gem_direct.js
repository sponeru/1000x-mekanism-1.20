...
ServerEvents.recipes(event => {
    ...
    //宝石系鉱石の倍化(汚い粉で止める)
    processing_gem_direct("diamond")
    processing_gem_direct("emerald")

    //通常鉱石の倍化処理
    function processing_gem_direct(material) {
        //倍化率
        var doubling_rate = {
            purifying_shard: 2,
            clean_washing: 10,
        ...
        //素材タグ
        var tag_material = {
            dirty_dust: `forge:gems/${material}`,
        ...
        function ore_process() {
            //濃縮室
            event.recipes.mekanism.enriching(tag_to_item(tag_material.dirty_dust, doubling_rate.enrich_ore), `#${ore.ore}`)
        ...
        function dense_ore_process() {
            //濃縮室
            event.recipes.mekanism.enriching(tag_to_item(tag_material.dirty_dust, doubling_rate.enrich_dense_ore), `#${ore.dense_ore}`)      
